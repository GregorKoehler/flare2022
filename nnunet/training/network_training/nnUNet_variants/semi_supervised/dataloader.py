...
class DataLoader3DSemi(SlimDataLoaderBase):
    def __init__(
        ...
        # order is important to be able to sample separately from the two data sets
        self.list_of_keys = list(self._data.keys())
        self.list_of_additional_keys = list(additional_data.keys())
        self._data.update(additional_data)
        self.additional_data_batch_size = int(additional_data_batch_share * batch_size)
    ...
    def generate_train_batch(self):
        ...
        selected_keys_additional_data = np.random.choice(
            self.list_of_additional_keys, self.additional_data_batch_size, True, None
        )
        selected_keys[
            self.batch_size - self.additional_data_batch_size :
        ] = selected_keys_additional_data
